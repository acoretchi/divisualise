...
export class BinarySearchCall extends RecursiveCall<BinarySearchInput, NumberValue> {
	...
	undividedDetails(): CallDetails {
		return [
			{
				text: `We want to find the index of the target value ${
				...
				valueKeyframes: [
					{
						Array: new NumberList(
							input.array.values.map((v, i) =>
								v
									.copy()
									.coloured(i < middleIndex ? 'gray' : i > middleIndex ? 'blue' : 'red')
									.struckThrough(i < middleIndex)
							)
						),
					...
					valueKeyframes: [
						{
							...
							Array: new NumberList(
								input.array.values.map((v, i) =>
									new NumberValue(v.value)
										.coloured(i === index ? 'green' : 'gray')
										.struckThrough(i !== index)
								)
							),
							Target: new NumberValue(input.target.value).coloured('green')
						}
			...
			return [
				{
					...
					valueKeyframes: [
						{
							Array: new NumberList(
								input.array.values.map((v) => v.copy().coloured('gray').struckThrough(true))
							),
							Target: new NumberValue(input.target.value).coloured('green')
						}
					]
		...
		return [
			{
				text: `The target value ${
					input.target.value
				} matches the element at the mid. We have found the target at index ${this.solve().value}.`,
				valueKeyframes: [
					{
						Array: new NumberList(
							input.array.values.map((v, i) =>
