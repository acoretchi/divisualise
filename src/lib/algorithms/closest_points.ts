...
export class ClosestPointsCall extends RecursiveCall<ClosestPointsInput, Points> {
	case(): DivideCase<ClosestPointsInput, Points> | BaseCase<ClosestPointsInput, Points> {
		...
		if (points.length <= 3) {
			...
			return new ClosestPointsDivideCase(
		...
		return [
			{
				...
				valueKeyframes: [
					{
						Points: new Points(points.map((p) => p.coloured(p.x <= midX ? 'blue' : 'red'))).addLine(
							new Line(new Point(midX, 0), new Point(midX, 1))
						)
					}
				],
		...
		return [
			leftPair.length > 1
						...
						valueKeyframes: [
							{
								Points: new Points(
									points.map((p) =>
										p.copy().coloured(leftPair.some((lp) => lp.equals(p)) ? 'red' : 'gray')
									)
				...
				: {
						...
						valueKeyframes: [
							{
								Points: new Points(points.map((p) => p.coloured('gray'))).addLine(
									new Line(new Point(midX, 0), new Point(midX, 1)).coloured('black')
								)
							}
						],
						highlightedCalls: ['Left']
						...
						valueKeyframes: [
							{
								Points: new Points(
									points.map((p) =>
										p.copy().coloured(rightPair.some((rp) => rp.equals(p)) ? 'blue' : 'gray')
									)
								).addLine(new Line(new Point(midX, 0), new Point(midX, 1)).coloured('black'))
							}
						],
				...
				? [
						{
							...
							valueKeyframes: stripPairs.map((pair) => ({
								Points: new Points(
									points
										.map((p) =>
											p.copy().coloured(Math.abs(p.x - midX) < minDist ? 'black' : 'gray')
										)
										.map((p) =>
											p.copy().coloured(pair.some((cp) => cp.equals(p)) ? 'purple' : p.colour)
										)
								)
									.addLine(new Line(new Point(midX, 0), new Point(midX, 1)).coloured('black'))
									.addLine(
										new Line(new Point(midX - minDist, 0), new Point(midX - minDist, 1)).coloured(
											'black'
										)
									)
									.addLine(
									...
									valueKeyframes: [
										{
											Points: new Points(
												points
													...
													.map((p) =>
														p
															.copy()
															.coloured(
																closestStripPair.some((cp) => cp.equals(p)) ? 'purple' : p.colour
															)
													)
											)
												.addLine(new Line(new Point(midX, 0), new Point(midX, 1)).coloured('black'))
												.addLine(
													new Line(
														new Point(midX - minDist, 0),
														new Point(midX - minDist, 1)
													).coloured('black')
												)
												.addLine(
													new Line(
				...
				valueKeyframes: [
					{
						'Closest Pair': new Points(
							points
								.map((p) => p.copy().coloured(leftPair.some((cp) => cp.equals(p)) ? 'red' : 'gray'))
								.map((p) =>
									p.copy().coloured(rightPair.some((cp) => cp.equals(p)) ? 'blue' : p.colour)
								)
								.map((p) =>
									p.copy().coloured(closestPair.some((cp) => cp.equals(p)) ? 'green' : p.colour)
								)
						)
					}
				],
							...
							'Closest Pair': new Points(
								points.map((p) =>
									p
										.copy()
										.coloured(
											p.equals(closestPair[0]) || p.equals(closestPair[1]) ? 'green' : 'gray'
										)
								)
							)
						}
