...
export class CofactorExpansionCall extends RecursiveCall<CofactorExpansionInput, NumberValue> {
	...
	case():
		| DivideCase<CofactorExpansionInput, NumberValue>
		| BaseCase<CofactorExpansionInput, NumberValue> {
...
export class CofactorExpansionDivideCase extends DivideCase<CofactorExpansionInput, NumberValue> {
	combine(): NumberValue {
		...
		for (let i = 0; i < this.input().matrix.matrix.length; i++) {
			...
			const term = new NumberValue(
				sign * this.input().matrix.matrix[0][i].value * cofactorValue.value
			);
		...
		for (let i = 0; i < matrix.matrix.length; i++) {
			...
			for (let j = 0; j < cofactor.matrix.length; j++) {
				for (let k = 0; k < cofactor.matrix[j].length; k++) {
					matrixWithColoredElements.matrix[j + 1][k < i ? k : k + 1] =
						cofactor.matrix[j][k].coloured(cofactorColor);
				}
			}
		...
		for (let i = 0; i < matrix.matrix.length; i++) {
			...
			details.push(
				...
				},
				{
					text:
						sign === 1
							? `We add this term to the determinant.`
							: `We subtract this term from the determinant.`,
					valueKeyframes: [
						{
