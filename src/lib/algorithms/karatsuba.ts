...
export class KaratsubaCall extends RecursiveCall<KaratsubaInput, NumberValue> {
	...
	case(): DivideCase<KaratsubaInput, NumberValue> | BaseCase<KaratsubaInput, NumberValue> {
		...
		if (x < 10 || y < 10) {
			...
			const n = Math.max(Math.floor(Math.log10(x)) + 1, Math.floor(Math.log10(y)) + 1);
			const half = Math.floor(n / 2);
			const divisor = Math.pow(10, half);

			const a = Math.floor(x / divisor);
			const b = x % divisor;
			const c = Math.floor(y / divisor);
			const d = y % divisor;
...
export class KaratsubaDivideCase extends DivideCase<KaratsubaInput, NumberValue> {
	combine(): NumberValue {
		...
		const n = Math.max(
			Math.floor(Math.log10(this.input().x.value)) + 1,
			Math.floor(Math.log10(this.input().y.value)) + 1
		);
		const half = Math.floor(n / 2);
		const divisor = Math.pow(10, half);
	...
	dividedDetails(input: KaratsubaInput): CallDetails {
		const n = Math.max(
			Math.floor(Math.log10(input.x.value)) + 1,
			Math.floor(Math.log10(input.y.value)) + 1
		);
		const half = Math.floor(n / 2);
		const divisor = Math.pow(10, half);
				...
				valueKeyframes: [
					{
						Result: new NumberValue(
							ac.value * Math.pow(10, 2 * half) + adPlusBC.value * divisor + bd.value
						),
