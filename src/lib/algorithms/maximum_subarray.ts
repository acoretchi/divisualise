...
export class MaximalSubarrayCall extends RecursiveCall<MaximalSubarrayInput, NumberList> {
	...
	case():
		| DivideCase<MaximalSubarrayInput, NumberList>
		| BaseCase<MaximalSubarrayInput, NumberList> {
				...
				valueKeyframes: [
					{
						Array: new NumberList(
							this.input().array.values.map((val) => new NumberValue(val.value))
						)
					}
				]
			}
...
export class MaximalSubarrayDivideCase extends DivideCase<MaximalSubarrayInput, NumberList> {
	combine(): NumberList {
		...
		if (
			sum(leftSubarray).value >= sum(rightSubarray).value &&
			sum(leftSubarray).value >= maxLeftSum + maxRightSum
		) {
			return leftSubarray;
		} else if (
			sum(rightSubarray).value >= sum(leftSubarray).value &&
				...
				valueKeyframes: [
					{
						...
						Right: new NumberList(
							input.array.values.slice(mid).map((val) => new NumberValue(val.value).coloured('red'))
						)
					}
				]
			}
		...
		return [
			...
			},
			{
				...
				valueKeyframes: [
					{
						'Crossing Subarray': new NumberList(
							crossingSubarray.values.map((val) => new NumberValue(val.value).coloured('purple'))
						)
					}
				],
				highlightedCalls: ['Left', 'Right']
			...
			},
			{
				...
				valueKeyframes: [
					{
						'Left Subarray': new NumberList(
							leftSubarray.values.map((val) => new NumberValue(val.value).coloured('blue'))
						),
			...
			},
			{
				...
				valueKeyframes: [
					{
						'Right Subarray': new NumberList(
							rightSubarray.values.map((val) => new NumberValue(val.value).coloured('red'))
						),
			...
			},
			{
				...
				valueKeyframes: [
					{
						'Maximal Subarray': new NumberList(
							this.result().values.map((val) => new NumberValue(val.value).coloured('green'))
						)
					}
				]
			}
		...
		return [
			{
				...
				valueKeyframes: [
					{
						Array: new NumberList(
							input.array.values.map((val) => new NumberValue(val.value).coloured('gray'))
						)
					}
				]
			}
		...
		return [
			{
				...
				valueKeyframes: [
					{
						'Maximal Subarray': new NumberList(
							this.solve().values.map((val) => new NumberValue(val.value).coloured('green'))
						),
						Array: new NumberList(
							input.array.values.map((val) => new NumberValue(val.value).coloured('gray'))
						)
