...
export class QuickSortDivideCase extends DivideCase<QuickSortInput, NumberList> {
	...
	solvedDetails(input: QuickSortInput): CallDetails {
		...
		const left = this.calls()
			['Left'].result()
			.values.map((v) => new NumberValue(v.value).coloured('blue'));
		const right = this.calls()
			['Right'].result()
			.values.map((v) => new NumberValue(v.value).coloured('red'));
				...
				valueKeyframes: [
					{
						Array: new NumberList(
							input.array.values.map((v) => new NumberValue(v.value).coloured('green'))
						)
					}
				]
			}
