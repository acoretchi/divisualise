...
export class StrassenCall extends RecursiveCall<StrassenInput, Matrix> {
	constructor(input: StrassenInput) {
		super(input);
		if (
			input.A.matrix.length !== input.A.matrix[0].length ||
			input.B.matrix.length !== input.B.matrix[0].length
		) {
		...
		return [
			{
				...
				valueKeyframes: [
					{
						C11: new Matrix(
							C11.map((row) => row.map((val) => new NumberValue(val.value).coloured('red')))
						),
			...
			},
			{
				...
				valueKeyframes: [
					{
						C12: new Matrix(
							C12.map((row) => row.map((val) => new NumberValue(val.value).coloured('blue')))
						),
			...
			},
			{
				...
				valueKeyframes: [
					{
						C21: new Matrix(
							C21.map((row) => row.map((val) => new NumberValue(val.value).coloured('yellow')))
						),
			...
			},
			{
				...
				valueKeyframes: [
					{
						C22: new Matrix(
							C22.map((row) => row.map((val) => new NumberValue(val.value).coloured('green')))
						),
						M1: this.calls()['M1'].result().copy(),
						M2: this.calls()['M2'].result().copy(),
						M3: this.calls()['M3'].result().copy(),
				...
				valueKeyframes: [
					{
						C: new Matrix([
							...C11.map((row, i) => [
								...row.map((val) => new NumberValue(val.value).coloured('red')),
								...C12[i].map((val) => new NumberValue(val.value).coloured('blue'))
							]),
							...C21.map((row, i) => [
								...row.map((val) => new NumberValue(val.value).coloured('yellow')),
								...C22[i].map((val) => new NumberValue(val.value).coloured('green'))
							])
						])
					}
				]
...
export class StrassenBaseCase extends BaseCase<StrassenInput, Matrix> {
	solve(): Matrix {
		...
		return new Matrix([
			[
				new NumberValue(A[0][0].value * B[0][0].value + A[0][1].value * B[1][0].value),
				new NumberValue(A[0][0].value * B[0][1].value + A[0][1].value * B[1][1].value)
			],
			[
				new NumberValue(A[1][0].value * B[0][0].value + A[1][1].value * B[1][0].value),
				new NumberValue(A[1][0].value * B[0][1].value + A[1][1].value * B[1][1].value)
			]
		]);
	}
