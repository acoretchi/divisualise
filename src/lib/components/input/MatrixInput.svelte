...
<div class="flex flex-col mb-4">
	...
	<div class="flex flex-col items-center">
		<div class="flex justify-center border-b-4 border-b-4 border-black pl-4 mb-4">
			...
			<input
				class="md:pl-3 text-xl md:text-3xl w-12 bg-transparent text-center drop-shadow outline-none"
				type="number"
				bind:value={n}
				on:change={onChangeN}
				min="2"
		...
		<div class="flex flex-col w-full overflow-x-auto">
			<div
				class="flex flex-col mx-auto w-fit items-center mb-2 md:mb-4 border-2 border-black overflow-x-auto"
			>
				{#each value.matrix as row, i}
					<div class="flex">
						{#each row as _, j}
							<div
								class="flex flex-col justify-center w-16 h-16 md:w-20 md:h-20 border-2 border-black"
							>
								<input
									type="number"
									class="bg-white text-2xl md:text-3xl h-full text-center md:pl-2"
									bind:value={value.matrix[i][j].value}
								/>
							</div>
						{/each}
					</div>
