			...
			{#each value.values as item, i}
				...
				>
					<input
						type="text"
						readonly
						class="bg-white text-2xl md:text-4xl w-full h-full text-center border-none ring-0 outline-none"
						value={item.value}
					/>
				</div>
			{/each}
			<div
				class="flex h-16 md:h-20 aspect-square items-center justify-center border-2 border-r-2 border-black"
			>
				<input
					type="text"
					class="bg-gray-300 text-2xl md:text-4xl w-full h-full text-center border-none ring-0 outline-none"
					bind:value={inputBuffer}
					on:input={onInput}
					on:keydown={deleteOnBackspace}
					on:click={() => {
						focusedIndex = null;
					}}
				/>
