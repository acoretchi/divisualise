...
{:else if value instanceof NumberList}
    <NumberListComponent value={value} />
{:else if value instanceof Points}
    <PointsComponent value={value} />
{:else if value instanceof Matrix}
    <MatrixComponent value={value} />
{:else if value && value.constructor === Object}
    {#each Object.entries(value) as [k, v]}
        <div>
            ...
            {/if}
            <span class="font-semibold text-xl md:text-2xl my-2 md:my-4" class:capitalize={capitalise}>{k}:</span>
            <div class="h-2" />
            <svelte:self value={v} />
            <div class="h-6" />
        </div>
